...
# Voice input (graceful if missing mic)
...
# -------------------------------------------------
# Load environment & debug
# -------------------------------------------------
load_dotenv()
API_KEY = os.getenv("GROQ_API_KEY", "")
print("DEBUG: API Key Loaded =", (API_KEY[:5] + "...") if API_KEY else None)
...
if "settings" not in st.session_state:
    st.session_state.settings = {"temperature": 0.2, "max_tokens": 512, "top_p": 1.0, "model": "llama3-8b-8192"}
...
print("DEBUG: GroqClient initialized with key =", (API_KEY[:5] + "...") if API_KEY else None)
...
with st.sidebar:
    ...
    # TTS toggle (app-level)
    st.subheader("Voice Output")
    tts_enabled = st.toggle("Speak responses (TTS)", value=False)
    ...
    if conversations:
        ...
        with st.expander("Manage conversation"):
            new_name = st.text_input("Rename to", value=st.session_state.current_conversation.get("name", "Conversation"))
            cols = st.columns(3)
...
with st.form("chat_input", clear_on_submit=True):
    ...
    uploaded = st.file_uploader("Upload document (txt, pdf, docx, png, jpg, csv, json)", type=["txt","pdf","docx","png","jpg","jpeg","csv","json"])
...
# Voice input (best-effort; requires PyAudio for Microphone)
if voice_clicked:
    try:
...
if submitted:
    if text_in.strip():
        ...
        # If a file is uploaded, extract text and append (limited to avoid 413)
        if uploaded:
            from utils.ocr import extract_text_from_file
            appended = extract_text_from_file(uploaded, filename=uploaded.name)
            if appended and appended.strip():
                # Append a doc snippet to the user prompt
                snippet = appended[:4000]  # keep under a safe limit
